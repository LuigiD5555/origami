...
func build(path, out string, dataWidth int) error {
	...
	payload, header, err := selfcontainedbook.Pack(text, compressed, uint32(len(doc.Pages)), uint32(blockCount), "bz2-9")
	if err != nil {
		return err
	}
	...
	decodedHeader, decodedCompressed, err := selfcontainedbook.Unpack(decodedPayload)
	if err != nil {
		return err
	}
	...
	exact := bytes.Equal(text, recovered) && sourceHash(recovered) == decodedHeader.SourceSHA256
	status := "PASS_ROUNDTRIP_PASS_512K"
	if rendered.OverTargetBytes > 0 {
		status = "PASS_ROUNDTRIP_FAIL_512K"
	}
	if !exact {
		status = "FAIL_ROUNDTRIP"
	}
	rep := report{
		...
		SemanticFoldClaim:  "Fixed Carrier R2 BOOT/ROSETTA/root index + canonical page/block address graph",
		ExactResidualClaim: "lossless bzip2 residual encoded in pixels; literal transport is not semantic Fold",
	}
...
func decode(path, out string) error {
	...
	header, compressed, err := selfcontainedbook.Unpack(payload)
	if err != nil {
		return err
	}
	...
	}{selfcontainedbook.Schema + ".decode", header, control, out, true})
}
