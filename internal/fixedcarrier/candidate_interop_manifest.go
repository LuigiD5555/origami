...
type VisibleTextManifest struct {
	Schema string `json:"schema"`
	ProgramSHA256 string `json:"program_sha256"`
	Facts []SemanticFact `json:"facts"`
}
...
type InteropBuildManifest struct {
	...
	VisibleText VisibleTextManifest `json:"visible_text"`
}
...
func TemporalSemanticManifest(decoded TemporalCarrierDecoded, mutations []CandidateMutation) SemanticManifest {
	...
	for _,c:=range decoded.Program.Automaton.Cells{
		facts=append(facts,
			SemanticFact{Key:"CELL."+c.ID+".INITIAL",Value:c.InitialState},
			SemanticFact{Key:"VISIBLE_CELL_ID_"+c.ID,Value:c.ID},
		)
	}
	...
	for _,m:=range mutations{
		...
		if t=="CELL_IDENTITY_ENCODING" && v==VisibleCellIdentityRedundancyR1{
			labels:=temporalVisibleCellLabels(decoded,true)
			for _,c:=range decoded.Program.Automaton.Cells{setFact(&facts,"VISIBLE_CELL_ID_"+c.ID,labels[c.ID])}
		}
	}
...
// TemporalVisibleTextManifest is a renderer declaration, not OCR. Every string
// is derived from the same structured TemporalProgram and mutation provenance
// used by the deterministic renderer. A future shadow OCR gate may verify pixels
// independently without changing this contract.
func TemporalVisibleTextManifest(decoded TemporalCarrierDecoded, mutations []CandidateMutation) VisibleTextManifest {
	...
	for _,c:=range decoded.Program.Automaton.Cells{
		facts=append(facts,
			SemanticFact{Key:"CELL."+c.ID+".LABEL",Value:"CELL "+labels[c.ID]},
			SemanticFact{Key:"CELL."+c.ID+".INITIAL_TEXT",Value:c.InitialState},
		)
	}
	if grammar {
		...
		if len(decoded.Program.Automaton.Cells)<=4{
			...
			for _,r:=range decoded.Program.Automaton.Rules{
				...
				if len(r.Requires)>0{
					...
					for _,pred:=range r.Requires{parts=append(parts,shortLabel(labels[pred.CellID],16)+"="+shortLabel(pred.State,10))}
					req=strings.Join(parts,"&")
				}
				from:=r.FromState;if strings.TrimSpace(from)==""{from="*"}
				line:=fmt.Sprintf("IF %s => %s:%s>%s",req,shortLabel(labels[r.TargetCell],16),shortLabel(from,10),shortLabel(r.ToState,10))
				facts=append(facts,SemanticFact{Key:"RULE."+r.ID+".TEXT",Value:shortLabel(line,94)})
				visible++
			}
		}
	}
...
func hasCandidateMutation(mutations []CandidateMutation,target,value string)bool{
	for _,m:=range mutations{if strings.EqualFold(strings.TrimSpace(m.Target),target)&&strings.EqualFold(strings.TrimSpace(m.Value),value){return true}}
	return false
}
