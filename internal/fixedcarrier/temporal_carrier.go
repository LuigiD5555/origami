...
func RenderTemporalCarrier(program temporal.Program) ([]byte, TemporalCarrierDecoded, error) {
	...
	visibleCells, visibleRules := drawTemporalSemanticPlane(img, program, nil)
...
func DecodeTemporalCarrierPNG(data []byte) (TemporalCarrierDecoded, error) {
	if len(data) > MaxPNGBytes {
		return TemporalCarrierDecoded{}, fmt.Errorf("carrier exceeds hard limit: %d", len(data))
	}
	img, err := png.Decode(bytes.NewReader(data))
	if err != nil {
		return TemporalCarrierDecoded{}, err
	}
	...
	if !bytes.Equal(rec[:8], temporalMagic[:]) {
		return TemporalCarrierDecoded{}, fmt.Errorf("temporal carrier magic mismatch")
	}
	if binary.BigEndian.Uint32(rec[508:512]) != crc32.ChecksumIEEE(rec[:508]) {
		return TemporalCarrierDecoded{}, fmt.Errorf("temporal carrier crc mismatch")
	}
	plainLen := int(binary.BigEndian.Uint32(rec[8:12]))
	compressedLen := int(binary.BigEndian.Uint16(rec[12:14]))
	if compressedLen < 0 || 46+compressedLen > 508 {
		return TemporalCarrierDecoded{}, fmt.Errorf("invalid compressed length")
	}
	...
	if len(plain) != plainLen {
		return TemporalCarrierDecoded{}, fmt.Errorf("program length mismatch: %d != %d", len(plain), plainLen)
	}
	digest := sha256.Sum256(plain)
	if !bytes.Equal(rec[14:46], digest[:]) {
		return TemporalCarrierDecoded{}, fmt.Errorf("program sha256 mismatch")
	}
	var program temporal.Program
	if err := json.Unmarshal(plain, &program); err != nil {
		return TemporalCarrierDecoded{}, err
	}
	if err := program.Validate(); err != nil {
		return TemporalCarrierDecoded{}, err
	}
	visibleCells := len(program.Automaton.Cells)
	if visibleCells > 8 {
		visibleCells = 8
	}
	visibleRules := len(program.Automaton.Rules)
	if visibleRules > 8 {
		visibleRules = 8
	}
...
func drawTemporalSemanticPlane(img *image.Gray, p temporal.Program, trace *temporal.Trace) (int, int) {
	...
	for _, r := range p.Automaton.Rules {
		...
		from, ok := positions[r.Requires[0].CellID]
		if !ok {
			continue
		}
	...
	for s := 0; s <= steps; s++ {
		x := x0 + (x1-x0)*s/steps
		h := 8
		if p.CheckpointEvery > 0 && s%p.CheckpointEvery == 0 {
			h = 18
			drawSmallRing(img, x, y+28)
		}
	...
	// If a trace is provided, render frozen checkpoint states below the timeline.
	if trace != nil {
		drawCheckpointStates(img, trace)
	}
...
func drawTemporalExactPlane(img *image.Gray, rec []byte) {
	...
	for y := 0; y < GridBits; y++ {
		for x := 0; x < GridBits; x++ {
			...
			if b == 1 {
				fill(img, x0+x*Cell, y0+y*Cell, Cell, Cell, 0)
			}
			bit++
		}
	}
...
func drawArrowTip(img *image.Gray, x0, y0, x1, y1 int) {
	...
	if absInt(dx) >= absInt(dy) {
		if dx >= 0 {
			drawSimpleLine(img, x1, y1, x1-6, y1-4)
			drawSimpleLine(img, x1, y1, x1-6, y1+4)
		} else {
			drawSimpleLine(img, x1, y1, x1+6, y1-4)
			drawSimpleLine(img, x1, y1, x1+6, y1+4)
		...
		if dy >= 0 {
			drawSimpleLine(img, x1, y1, x1-4, y1-6)
			drawSimpleLine(img, x1, y1, x1+4, y1-6)
		} else {
			drawSimpleLine(img, x1, y1, x1-4, y1+6)
			drawSimpleLine(img, x1, y1, x1+4, y1+6)
