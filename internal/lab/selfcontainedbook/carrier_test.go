...
func TestPackRenderDecodeRoundTrip(t *testing.T) {
	...
	payload, header, err := Pack(source, compressed, 12, 34, "test")
	if err != nil {
		t.Fatal(err)
	}
	...
	rendered, err := Render(controlPNG, payload, 640)
	if err != nil {
		t.Fatal(err)
	}
	...
	decodedHeader, gotCompressed, err := Unpack(decodedPayload)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnpackRejectsCorruption(t *testing.T) {
	...
	payload, _, err := Pack(source, compressed, 1, 1, "test")
	if err != nil {
		t.Fatal(err)
	}
